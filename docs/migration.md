...
Canonical receipt and manifest v2 added `declared_input_identity`. Canonical v3
adds `receipt_identity` and replaces serializer-defined manifest identity with
versioned schema-owned canonical receipt and manifest bytes. Consumers may use
the declared identity to correlate repeated evaluations or detect differing
outputs for the same declared inputs. They must not use a `declared_only` or `snapshot_only`
